...
// 14/09/2026: a limpeza manual de 26 mensagens (horas/dias depois de postadas) saiu SEM bolha vazia,
// e as 3 exclusoes automaticas de 13/09 (3 a 32 segundos depois da mensagem) sairam COM bolha, mandando
...
if (podeApagar) {
  ...
  // (teste 1 no QA, 11/09). Telefone (participantAlt) resolve sempre; o @lid fica so como reserva.
  const autorJid = ctx.telefone ? (ctx.telefone + '@s.whatsapp.net') : ctx.participant;
  // Espera a mensagem envelhecer: o webhook ja respondeu, entao segurar a execucao aqui nao trava nada.
  const idade = Math.floor(Date.now() / 1000) - Number(ctx.ts || 0);
  const faltam = ESPERA_MIN - idade;
  if (faltam > 0 && faltam <= ESPERA_MIN) { await new Promise((ok) => setTimeout(ok, faltam * 1000)); }
