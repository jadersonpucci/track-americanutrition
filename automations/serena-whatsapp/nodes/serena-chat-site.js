...
const CSS = ':root{--an-azul:#1f4fd8;--an-esc:#0e1726}'
  ...
  + '#anchat-h{background:var(--an-esc);color:#fff;padding:13px 15px;display:flex;align-items:center;gap:10px}'
  ...
  + '#anchat-m{flex:1;overflow-y:auto;padding:14px;background:#f5f6f8}'
  ...
  + '#anchat-f{border-top:1px solid #e3e6ec;padding:9px;display:flex;gap:8px;background:#fff}'
  + '#anchat-t{flex:1;border:1px solid #d8dce4;border-radius:20px;padding:9px 14px;font:15px/1.35 inherit;resize:none;max-height:96px;outline:0}'
  ...
  + '@media(max-width:520px){#anchat-p{right:0;bottom:0;width:100vw;max-width:100vw;height:100dvh;max-height:100dvh;border-radius:0}#anchat-b{right:14px;bottom:14px}}';

const JS = '(function(){if(window.__anchat)return;window.__anchat=1;'
  + 'var BASE=' + JSON.stringify(BASE) + ';'
  + 'var css=' + JSON.stringify(CSS) + ';'
  + 'var st=document.createElement("style");st.textContent=css;document.head.appendChild(st);'
  + 'var s=localStorage.getItem("an_chat_s");if(!s){s=Math.random().toString(36).slice(2,10)+Date.now().toString(36).slice(-4);localStorage.setItem("an_chat_s",s);}'
  + 'var ultimo=localStorage.getItem("an_chat_u")||"0";'
  ...
  + 'var naoLidas=0,aberto=false,timer=null;'
  ...
  + 'function salvar(){try{localStorage.setItem("an_chat_h",JSON.stringify([].slice.call(mm.children).map(function(e){return{q:e.className.indexOf("eu")>0?"eu":"ela",t:e.textContent};}).slice(-40)));}catch(e){}}'
  + 'try{var h=JSON.parse(localStorage.getItem("an_chat_h")||"[]");h.forEach(function(m){bolha(m.q,m.t);});}catch(e){}'
  + 'if(!mm.children.length){bolha("ela","Oi! Sou a Serena, do atendimento da America Nutrition \\ud83d\\udc99 Como posso ajudar voc\\u00ea?");}'
  + 'function abrir(){p.classList.add("on");aberto=true;naoLidas=0;pt.style.display="none";fim();tx.focus();poll();}'
  + 'function fechar(){p.classList.remove("on");aberto=false;}'
  ...
  + 'if(j.ultimo_id){ultimo=j.ultimo_id;localStorage.setItem("an_chat_u",ultimo);}'
  ...
  + 'if(j.msgs&&j.msgs.length){j.msgs.forEach(function(m){if(m.texto){bolha("ela",m.texto);if(!aberto){naoLidas++;pt.textContent=naoLidas;pt.style.display="block";}}ultimo=m.id;});localStorage.setItem("an_chat_u",ultimo);salvar();}'
  + '}catch(e){}}'
