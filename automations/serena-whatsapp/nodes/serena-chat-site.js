...
// Aponta para a conta do Samuel (Telegram Business), nao para o bot: a conversa nasce
// dentro do app da equipe e a resposta sai em nome dele, nao em nome de um bot.
const TELEGRAM = 'https://t.me/AmericaNutrition';
...
if (acao === 'enviar') {
  ...
  if (cnt.length && Number(cnt[0].n) >= LIMITE_HORA) {
    return json({ resposta: 'Voc\u00ea mandou muitas mensagens seguidas. Espere alguns minutos e continue, ou escreva para contato@americanutrition.com.', limite: true });
  }
  ...
  if (novo) {
    ...
    await req({ method: 'POST', url: TG, headers: { 'Content-Type': 'application/json' }, json: true, timeout: 10000,
      body: { chat_id: TG_GRUPO, message_thread_id: TG_TOPICO, parse_mode: 'HTML', disable_web_page_preview: true,
  ...
  if (r.pausada === true) {
    return json({ resposta: '', pausada: true, contato_id: contatoId, aviso: 'Um atendente est\u00e1 acompanhando esta conversa e responde por aqui.' });
  }
...
const CSS = ':root{--an-azul:#07388e;--an-link:#1f4fd8;--an-tg:#229ED9}'
  ...
  + '#anchat-tgb{flex:0 0 auto;display:flex;align-items:center;gap:9px;background:#eaf6fd;border-bottom:1px solid #d3e9f7;color:#0f5f87;text-decoration:none;font-size:12.5px;line-height:1.35;padding:9px 14px}'
  + '#anchat-tgb svg{width:17px;height:17px;fill:var(--an-tg);flex:0 0 17px}'
  + '#anchat-tgb b{font-weight:600}'
  + '#anchat-tgb.off{display:none}'
...
const JS = '(function(){if(window.__anchat)return;window.__anchat=1;'
  + 'var BASE=' + JSON.stringify(BASE) + ',TELEGRAM=' + JSON.stringify(TELEGRAM) + ';'
  + 'var css=' + JSON.stringify(CSS) + ',svgTg=' + JSON.stringify(SVG_TG) + ';'
  ...
  + 'var fab=document.createElement("div");fab.id="anchat-fab";'
  + 'var tgb=document.createElement("a");tgb.id="anchat-tg";tgb.href=TELEGRAM;tgb.target="_blank";tgb.rel="noopener";tgb.setAttribute("aria-label","Falar pelo Telegram");tgb.innerHTML=svgTg;'
  ...
  + 'fab.appendChild(tgb);fab.appendChild(b);'
  + 'var p=document.createElement("div");p.id="anchat-p";'
  + 'p.innerHTML=\'<div id="anchat-h"><i class="av">AN</i><div class="nm"><b>America Nutrition</b><span><i class="dot"></i>Atendimento \\u00b7 respondemos por aqui</span></div><button class="x" aria-label="Fechar">\\u00d7</button></div><a id="anchat-tgb" target="_blank" rel="noopener">\'+svgTg+\'<span>Prefere <b>Telegram</b>? L\\u00e1 voc\\u00ea recebe a resposta no celular, mesmo depois de sair do site.</span></a><div id="anchat-m"></div><div id="anchat-f"><textarea id="anchat-t" rows="1" placeholder="Escreva sua mensagem..."></textarea><button id="anchat-s" aria-label="Enviar"><svg viewBox="0 0 24 24"><path d="M2 21l21-9L2 3v7l15 2-15 2z"/></svg></button></div>\';'
  + 'document.body.appendChild(fab);document.body.appendChild(p);'
  + 'var mm=p.querySelector("#anchat-m"),tx=p.querySelector("#anchat-t"),bs=p.querySelector("#anchat-s"),pt=b.querySelector(".pt"),faixa=p.querySelector("#anchat-tgb");'
  + 'faixa.href=TELEGRAM;'
  ...
  + '/* a faixa do Telegram so aparece antes da conversa comecar: trocar de canal no meio perde o historico */'
  + 'function faixaTg(){var n=mm.querySelectorAll(".anm").length;if(n>1)faixa.classList.add("off");else faixa.classList.remove("off");}'
  + 'function bolha(quem,texto){var d=document.createElement("div");d.className="anm "+quem;var x=document.createElement("div");x.className="anb";x.innerHTML=linkar(texto);d.appendChild(x);mm.appendChild(d);fim();faixaTg();return d;}'
  + 'function salvar(){try{lsSet("an_chat_h",JSON.stringify([].slice.call(mm.querySelectorAll(".anm")).map(function(e){return{q:e.className.indexOf("eu")>0?"eu":"ela",t:e.textContent};}).slice(-40)));}catch(e){}}'
  ...
  + 'salvar();}catch(e){t.remove();bolha("ela","N\\u00e3o consegui enviar. Confira sua conex\\u00e3o e tente de novo.");}'
  + 'bs.disabled=false;}'
...
html += '<p>Nosso WhatsApp est\u00e1 temporariamente indispon\u00edvel. Voc\u00ea pode falar com a gente por aqui mesmo, sem instalar nada.</p>';
html += '<p>Toque no bal\u00e3o azul no canto da tela para come\u00e7ar, ou no bot\u00e3o do Telegram se preferir receber as respostas no celular.</p></div>';
