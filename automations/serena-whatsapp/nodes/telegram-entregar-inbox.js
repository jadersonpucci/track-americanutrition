...
const pend = await sql("select m.id::text as id, m.texto, substr(c.session_site, 4) as chat_id from serena_mensagens m join serena_contatos c on c.id = m.contato_id where m.entregue is distinct from true and m.canal = 'telegram' and m.papel in ('humano','serena') and nullif(m.texto,'') is not null and m.criado_em > now() - interval '2 days' and m.criado_em < now() - interval '20 seconds' and c.session_site like 'tg-%' order by m.criado_em limit 20");
...
for (const p of pend) {
  ...
  const r = await req({ method: 'POST', url: BOT + 'sendMessage', headers: { 'Content-Type': 'application/json' }, body: { chat_id: p.chat_id, text: texto, disable_web_page_preview: false }, json: true, timeout: 30000 });
