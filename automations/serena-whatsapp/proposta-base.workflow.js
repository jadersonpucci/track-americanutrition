...
const corpo = { model: cfg.modelo, max_tokens: 6000, system: system, messages: [{ role: 'user', content: user }] };
...
const txt = ((r && r.content) || []).filter(c => c.type === 'text').map(c => c.text).join('').trim();
const m = txt.match(/\\{[\\s\\S]*\\}/);
if (!m) return [{ json: { ok: false, erro: 'resposta sem JSON', bruto: txt.slice(0, 500) } }];
...
for (const it of itens) {
  t += (ICON[it.tipo] || '') + ' <b>' + it.n + '. ' + esc(it.titulo) + '</b> <i>(' + it.tipo + (it.secao ? ' \\u00b7 ' + esc(it.secao) : '') + ')</i>' + NL + esc(it.texto.length > 350 ? it.texto.slice(0, 350) + '\\u2026' : it.texto) + NL + '<i>Motivo: ' + esc(it.motivo) + '</i>' + NL + NL;
}
t += 'Aprovar entra em serena_config.system_prompt (adendos), que a Serena l\\u00ea junto com a base e o sync do site n\\u00e3o apaga. Depois vale copiar para o documento da base.';
