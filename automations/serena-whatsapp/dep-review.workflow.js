// n8n Workflow SDK — [Depoimentos] Review de Grupo (GET /webhook/dep-review)
// Botao do card do Radar abre: ?t=TOKEN&id=<grupo_radar.id>. Mostra o depoimento, deixa editar
// nome/texto/produto/nota e publica em reviews (origem whatsapp). Avisa quando ha alegacao terapeutica.
import { workflow, node, trigger, sticky } from '@n8n/workflow-sdk';

const entrada = trigger({ type: 'n8n-nodes-base.webhook', version: 2.1, config: { name: 'Link do Telegram', parameters: { httpMethod: 'GET', path: 'dep-review', responseMode: 'responseNode', options: {} } }, output: [{ query: { t: 'x', id: '1' } }] });

const agir = node({ type: 'n8n-nodes-base.code', version: 2, config: { name: 'Pagina ou Publicar', parameters: { jsCode: `const SK = 'SUPABASE_SERVICE_KEY';
...
const nota = sticky('## Depoimento de grupo vira review\n\nGET /webhook/dep-review?t=TOKEN&id=<grupo_radar.id>. O botao no card DEPOIMENTO ESPONTANEO do Radar abre esta pagina: mostra a mensagem original, sugere nome curto (Marcia G.) e texto, deixa editar, escolher produto e nota, e publica em reviews com origem whatsapp, verified false e o texto original guardado em ia_flags. Avisa quando o texto tem alegacao terapeutica (ANVISA). Marca grupo_radar.decisao para nao publicar duas vezes.', [entrada, agir], { color: 6 });

export default workflow('depoimentos-review-grupo', '[Depoimentos] Review de Grupo', { settings: { executionOrder: 'v1' } })
