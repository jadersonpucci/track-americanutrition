...
select (select coalesce(json_object_agg(chave, valor), '{}'::json) from checkout_config) as cfg,
       ...
       (select row_to_json(c) from checkout_pix_inter c where c.txid = nullif($10, '')) as cobranca,\n       (select row_to_json(b) from checkout_boleto_inter b where b.codigo_solicitacao = nullif($10, '')) as cobranca_boleto`, [1040, Y2 - 100],
